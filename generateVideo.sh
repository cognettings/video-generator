...
node scrapeImageUrls.js "$1" $2 | wget --directory-prefix="$1" -i - &&

# resize the images
mogrify -format jpg -resize 1280x720 -gravity center -background black -extent 1280x720 ./"$1"/* &&

# download audio
# node downloadAudio.js "$1" &&
curl -d "{\"query\":\"$1\"}" -H "Content-Type: application/json" -X POST http://10.0.0.34:3000/main -o "$1"/audio &&
